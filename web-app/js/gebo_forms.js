...
gebo_mask_input = {
    init: function() {
        ...
        $("#datepreselection").inputmask("99/99/9999", {placeholder: "dd/mm/yyyy"});
        $("#datereaudition").inputmask("99/99/9999", {placeholder: "dd/mm/yyyy"});
        $("#datediffusion").inputmask("99/99/9999", {placeholder: "dd/mm/yyyy"});
        $("#datepresentation").inputmask("99/99/9999", {placeholder: "dd/mm/yyyy"});
...
//                        $("#dateDebut").val(day+"/"+mois+"/"+currentTime.getFullYear());
//                        $("#dateFin").val(day+"/"+mois+"/"+currentTime.getFullYear());
...
gebo_datepicker = {
    init: function() {
        ...
        $('.dateTimeAudition').datetimepicker({
...
var start_date = $('#dp_start input').val();
var term = $('#duree').val();  // Is text value
...
var newmonth = (month + parseInt(term));
// Must convert term to integer
...
calcval1 = (((day <= 9) ? "0" + day : day) + "/" + ((month <= 9) ? "0" + month : month) + "/" + year);      
            $('#dp_end').datepicker('setStartDate', dateText);
            $('#dp_start').datepicker('hide');
            $('#dp_end input').val(calcval1);
           $("input[name='city']").val(calcval1);
        ...
        if($('#dp_start input').val()!==null && $('#dp_start input').val()!=='null' && $('#dp_start input').val()!=='')
            {
                var calcval = null;
...
gebo_tag_handler = {
    init: function() {
        $(".array_tag_handler").tagHandler({
            assignedTags: ['C', 'C++', 'C#', 'Java', 'Perl', 'PHP', 'Python'],
        });
    }
};
...
gebo_wysiwg = {
    init: function() {
        ...
        $('textarea#templateEtape2').tinymce({
            // Location of TinyMCE script
            script_url: 'lib/tiny_mce/tiny_mce.js',
            // General options
            theme: "advanced",
