...
function localisationAgents() {
    ...
    $.ajax({
        type: "POST",
        url: "/Massar/utilisateur/listAgents",
        dataType: 'json',
        success: function(data) {
            $.each(data, function(key, value) {
                origin1 = value.latLng.split(',');// += "[" + value.latLng.split(',') + "] ,";
                originsObjet.push(new google.maps.LatLng(origin1[0], origin1[1]));
                origins.push("{lat:" + origin1[0] + ",lng:" + origin1[1] + ", data:'" + value.nom + " " + value.prenom + "<br/>" + value.gsm + "', tag : 'agent', id : " + value.id + ", options : {icon:'/Massar/images/markers/ducati-diavel.png',draggable: false}}");
                agents.push(value);
            });
...
function calcul(response, status) {
    if (status == google.maps.DistanceMatrixStatus.OK) {
        ...
        origins[index] = "{lat:" + origin1[0] + ",lng:" + origin1[1] + ", data:'" + agents[index].nom + " " + agents[index].prenom + "<br/>" + agents[index].gsm + "', tag : 'agent', id : " + agents[index].id + ", options : {icon:'/Massar/images/markers/ducati-diavel-green.png',draggable: false,animation: google.maps.Animation.BOUNCE}}";
            ...
            $('html,body').animate({scrollTop: $('.main_content').offset().top - 40}, 'fast', function() {
                        ...
                        },
                {
                    ...
                    marker: {
                        options: {
                            draggable: true,
                            icon: new google.maps.MarkerImage("/Massar/images/markers/office-building.png")
                        },
                        events: {
